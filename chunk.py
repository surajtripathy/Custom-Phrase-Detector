...
import re
##Check for important keywords like to for of at on etc for start and end of phrases
def chunked(sentence):
    try:
        ...
        pass
##Check if more than two start/end phrases are present
def check_again_chunk(sentence):
    ...
    sent_at = re.search('at ', sentence)
    sent_need = re.search('need ',sentence)
    if sent_to:
